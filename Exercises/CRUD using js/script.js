...
let editData = (edit)=>{
    ...
    let row = edit.closest('tr');
    //console.log(row);
    let name = row.querySelector('td:first-child').innerHTML;
    let surname = row.querySelector('td:nth-child(2)').innerHTML;
    let address = row.querySelector('td:nth-child(3)').innerHTML;
    //console.log(name);
    //console.log(surname);
    //console.log(address);
    let editst = prompt("enter your first  name",name);
    if(editst !== '') {
        let oldName =table.querySelector("tr").children[0];
        let newName = document.createTextNode(editst);
        console.log(oldName);
        console.log(newName);
        oldName.replaceChild(newName, oldName.childNodes[0]);
        //console.log(x);

    }else{
        alert('Enter your name');
    }
    //let x = editst.value = "ok";
    //console.log(x);    
}   
